        ...
        if($debugc){
            ...
            if($platform -eq "Steam"){
                ...
                if($cpun -eq 1){
                    ...
                    $ps1script += "legendary auth --import; legendary -y uninstall Among Us --keep-files ; legendary -y import 'Among Us' 'C:\Program Files\Epic Games\AmongUs SNR Mod' ; legendary -y egl-sync ; legendary launch Among Us ; Start-sleep -seconds 5 ; Get-Process -Name 'Among Us' | % { .ProcessorAffinity=1;}`r`n"
                    $ps1name = "$dsk\amongusrun_$scid2.ps1"
                    ...
                    $ps1script += "Start-Process pwsh.exe -ArgumentList `"-NoProfile -ExecutionPolicy Bypass -WindowStyle Minimized -File `'$dsk\amongusrun_$scid2.ps1`'`" -Verb RunAs -Wait`r`n"
                    $ps1script += "exit`r`n}`r`n"
                    $ps1script += "legendary auth --import; legendary -y uninstall Among Us --keep-files ; legendary -y import 'Among Us' 'C:\Program Files\Epic Games\AmongUs SNR Mod' ; legendary -y egl-sync ; legendary launch Among Us ; Start-sleep -seconds 5 ; Get-Process -Name 'Among Us' | % { .ProcessorAffinity=3;}`r`n"
                    $ps1name = "$dsk\amongusrun_$scid2.ps1"
                    $ps1script | Out-File -Encoding "UTF8BOM" -FilePath "$ps1name" 
                    $sShortcut.Arguments = "-Command Start-Process '$dsk\startamongusrun_$scid2.bat'"
                }else{
